...
import comp4004.library.network.Client;
...
public class LibServer implements Runnable{
	...
	private Logger logger = UtilTrace.getInstance().getLogger(this);
	SInHandler handler=new SInHandler();
	...
	private void addThread(Socket socket) {
		...
		if (clientCount < UtilConfig.MAX_CLIENTS) {
			try {
	...
	public synchronized void handle(int ID, String input) {
		...
		else 
		{
			...
			if(exist(from)){
				...
				Client client=new Client(from,SInHandler.WAIT);
				clientList.add(client);
				so=handler.processInput(input,SInHandler.WAIT);
				output=so.getOutput()+"\n";
				from.send(output);
				clientSetState(from,so.getState());
				logger.info(String.format("Output to %s:%d"+" "+output,from.getSocketAddress(),from.getID()));
